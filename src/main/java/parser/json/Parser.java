...
public interface Parser {
	...
	public static final Parser ALL = empty(trimmed(nil(bool(string(integer(number(array(json(fail("unknown Parser"))))))))));
	...
	interface Value {
		...
		final static class ArrayValue implements Value {
			...
			@Override public Value isArray(final Consumer<Value> action) {
		...
		final static class ErrorValue implements Value {
			...
			ErrorValue (final String msg) { this.msg = msg; }
			@Override public Value isError(final Consumer<String> action) {
				action.accept(msg);
				return this;
			}
		}		
